  ...
  return(
    <div className="browse">
      <div className="browse-header">
        ...
        <div className="browse-filters">
          <div className="search-wrap">
            <svg className="search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="15" height="15"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
      ...
      <div className="browse-results">
        {items.length===0?<div className="state-msg"><span>🔍</span><p>No results{search?` for "${search}"`:''}.</p></div>:(
          <><div className="results-count">{items.length} item{items.length!==1?'s':''}</div>
          <div className="card-list">{items.map(item=><ContentCard key={item.id} item={item} onDownload={download} onRemove={remove} downloaded={downloaded[item.id]} downloading={downloading[item.id]}/>)}</div></>
        )}
      </div>
    </div>
  )
