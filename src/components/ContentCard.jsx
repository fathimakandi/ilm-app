...
export default function ContentCard({ item, onDownload, onRemove, downloaded, downloading, row = false }) {
  ...
  if (row) {
    return (
      <div className={`track-row${isActive ? ' active' : ''}`} onClick={handlePlay}>
        <div className={`track-art ${item.type}`}>
          {item.type === 'audio' ? '🎙️' : '📖'}
          {isActive && isPlaying
            ? <div className="track-eq"><span/><span/><span/></div>
            : <div className="track-play-ov"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></div>
          }
        </div>
        <div className="track-info">
          <div className="track-title">{item.title}</div>
          <div className="track-sub">{item.source}</div>
        </div>
        <div className="track-right">
          {isDown && <span className="track-saved">✓</span>}
  ...
  return (
    <div className={`content-card${isActive ? ' active' : ''}`} onClick={handlePlay}>
      ...
      <div className="card-body">
        ...
        <div className="card-desc">{item.source}</div>
      </div>
